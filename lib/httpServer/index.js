...
class HttpServer {
  ...
  async createEventWalletTransaction(eventID, publicKey, walletIdList) {
    return new Promise((resolve, reject) => {
      if (!Array.isArray(walletIdList)) {
      ...
      let transaction = {
        ...
        eventId: eventID,
        publicKey: publicKey,
        outputs: walletIdList.map(id => ({address: id, amount: 1})) // Add outputs
      };
