...
class HttpServer {
  constructor(node, blockchain, operator, miner) {
    ...
    this.app.post("/operator/wallets", async (req, res) => {
      ...
      if (eventID) {
        //event wallet
        let existingWalletOnBlockChain = operator.getRegisteredWalletByEventId(eventID);
        let existingWalletLocally = operator.getWalletById(eventID);
        if (existingWalletOnBlockChain != null || existingWalletLocally) {
          res
              .status(409)
        ...
        //Create new address
        try {
          const newAddress = await operator.generateAddressForWallet(walletId);
          let publicKey = newAddress.publicKey;
          let secretKey = newAddress.privateKey;
          if (!secretKey) {
            throw new Error("Secret key is undefined");
          /*} else {
            console.log("Secret key is available: " + secretKey);
          */}
          if (!walletId || !publicKey) {
            throw new Error("Wallet ID or public key is undefined");
          }
        ...
        //Check wallet with this studentID locally
        let existingWalletLocally = operator.getWalletById(studentID);
        //Check wallet with this studentID on blockchain
        let existingWalletOnBlockchain = operator.getRegisteredWalletByStudentId(studentID);
        if (existingWalletLocally != null || existingWalletOnBlockchain) {
          res.status(409).send({
  ...
  async createEventWalletTransaction(operator, eventID, publicKey, studentIDList) {
    return new Promise(async (resolve, reject) => {
      if (!Array.isArray(studentIDList)) {
        return reject(new TypeError("walletIdList must be an array"));
      }
      try {
        // Fetch the public keys for each student ID
        let publicKeyPromises = studentIDList.map(async studentID => {
          return operator.getPublicKeyByStudentId(studentID);
        });

        let publicKeys = (await Promise.all(publicKeyPromises)).filter(pk => pk !== null);
        ...
        let transaction = {
          ...
          eventId: eventID,
          publicKey: publicKey,
          outputs: publicKeys.map(pk => ({ address: pk, amount: 1 })) // Add outputs
        };
