...
class Blockchain {
    ...
    checkBlock(newBlock, previousBlock, referenceBlockchain = this.blocks) {
        ...
        // Include outputs of create_event_wallet transactions in the sum of inputs amount
        let eventWalletOutputs = R.sum(R.flatten(R.map(R.compose(R.map(R.prop('amount')), R.prop('outputs'), R.prop('data')), R.filter(tx => tx.type === 'create_event_wallet', newBlock.transactions))));
        sumOfInputsAmount += eventWalletOutputs;
